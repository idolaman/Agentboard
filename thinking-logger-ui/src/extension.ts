...
class SessionsViewProvider implements vscode.WebviewViewProvider {
	...
	async resolveWebviewView(webviewView: vscode.WebviewView): Promise<void> {
		...
		// Mock mode for screenshots/demo
		const MOCK = true;
		const nowMs = Date.now();
		const mockSessionsRaw: any[] = [
			{ id: 'cur-1', title: 'Add unit tests to core modules', platform: 'cursor', project: 'productivity-master', git_branch: 'main', started_at: new Date(nowMs - 90_000).toISOString() },
			{ id: 'cur-2', title: 'Set up workspace — install dependencies', platform: 'cursor', project: 'productivity-master', git_branch: 'main', started_at: new Date(nowMs - 140_000).toISOString(), approval_pending_since: new Date(nowMs - 20_000).toISOString() },
			{ id: 'gpt-1', title: 'Write pull‑request summary', platform: 'chatgpt', started_at: new Date(nowMs - 360_000).toISOString(), ended_at: new Date(nowMs - 30_000).toISOString() },
			{ id: 'gh-1', title: 'Build pipeline — main', platform: 'github', started_at: new Date(nowMs - 720_000).toISOString(), ended_at: new Date(nowMs - 300_000).toISOString() }
		];
		...
		async function pushOnce() {
			try {
				if (MOCK) {
					webviewView.webview.postMessage({ type: 'sessions', sessions: mockSessionsRaw.map((s: any) => ({
						id: s.id,
